...
from app.models.enums import ClassSessionStatus, ScheduleExceptionScope
...
class ClassSession(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("end_time > start_time", name="time_order"),
        CheckConstraint("configured_capacity > 0", name="configured_capacity_positive"),
        CheckConstraint("effective_capacity > 0", name="effective_capacity_positive"),
        CheckConstraint(
            "effective_capacity <= configured_capacity",
            name="effective_capacity_not_above_configured",
        ),
        CheckConstraint(
            "status <> 'PUBLISHED' OR teacher_id IS NOT NULL",
            name="published_requires_teacher",
        ),
        CheckConstraint(
            "status IN ('DRAFT', 'PUBLISHED', 'CANCELLED')",
            name="status_valid",
        ),
        UniqueConstraint("id", "organization_id", name="uq_class_sessions_id_org"),
        UniqueConstraint(
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    organization_id: Mapped[int] = mapped_column(Integer, index=True)
    source_template_id: Mapped[int] = mapped_column(Integer, index=True)
    generation_batch_id: Mapped[str] = mapped_column(String(36), index=True)
    branch_id: Mapped[int] = mapped_column(Integer, index=True)
    room_id: Mapped[int] = mapped_column(Integer, index=True)
    level_id: Mapped[int] = mapped_column(Integer, index=True)
    teacher_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    ...
    effective_capacity: Mapped[int] = mapped_column(Integer)
    status: Mapped[str] = mapped_column(
        String(20), default=ClassSessionStatus.DRAFT.value, index=True
    )
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_by_user_id: Mapped[int] = mapped_column(Integer)
    updated_by_user_id: Mapped[int] = mapped_column(Integer)

    source_template: Mapped["ScheduleTemplate"] = relationship(overlaps="branch,room,level")
    branch: Mapped["Branch"] = relationship(overlaps="source_template,room,level")
    room: Mapped["Room"] = relationship(overlaps="branch,source_template,level")
    level: Mapped["AcademicLevel"] = relationship(overlaps="branch,room,source_template")
    teacher: Mapped["TeacherProfile | None"] = relationship(
        overlaps="branch,level,room,source_template"
    )
    created_by: Mapped["User"] = relationship(
        foreign_keys=[created_by_user_id], overlaps="updated_by"
    )
